    ...
    <div>
        <div class="form-group row">
            <div class="offset-md-1 col-sm-2 text-center">
                <p><b>Таблица: Pany</b> {{ page.current }} / {{ page.count }}</p>
                <p>!Сверка Пени.sql</p>
            </div>
...
<script>
    export default {
        ...
        mounted() {
            this.page.count = Math.ceil(this.$store.getters.getArrayPeny.length / this.page.length);
        },
        computed: {
            arr() {
                if (this.search.length >= 9) {
                    return this.$store.getters.getArrayPeny.filter((item) => (item['Лицевой счет'].indexOf(this.search) !== -1));
                } else {
                    return this.$store.getters.getArrayPeny.filter((row, index) => {
        ...
        methods: {
            ...
            nextPage() {
                if ((this.page.current * this.page.length) < this.$store.getters.getArrayPeny.length) {
                    this.page.current += 1;
                }
            },
        }
