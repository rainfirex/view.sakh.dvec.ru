    ...
    <div>
        <div class="form-group row">
            <div class="offset-md-1 col-sm-2 text-center">
                <p><b>Таблица: OSB</b> {{ page.current }} / {{ page.count }}</p>
                <p>!Сверка и ОСВ.SQL</p>
            </div>
...
<script>
    export default {
        ...
        mounted() {
            this.page.count = Math.ceil(this.$store.getters.getArrayOSB.length / this.page.length);
        },
        computed: {
            arr() {
                if (this.search.length >= 9) {
                    return this.$store.getters.getArrayOSB.filter((item) => (item['Лицевой счет'].indexOf(this.search) !== -1));
                } else {
                    return this.$store.getters.getArrayOSB.filter((row, index) => {
        ...
        methods: {
            ...
            nextPage() {
                if ((this.page.current * this.page.length) < this.$store.getters.getArrayOSB.length) {
                    this.page.current += 1;
                }
            },
        }
